...
const services = [
{title:'Build something better.',category:'DEVELOPMENT',text:'From ambitious idea to beautifully engineered product. Your dedicated developers, already in sync.',tags:['Full-stack development','Web experiences','API integrations'],symbol:'↗'},
...
<div className="sculpture" aria-hidden="true"><div className="orbital-grid"/><div className="orbit"><i/><b/></div><div className="mark-tilt"><img className="mark-float" src="/brainboys-mark.png" alt=""/></div><div className="orbit-label label-top"><i/> GREAT MINDS, CONNECTED</div><div className="orbit-label label-bottom"><span>↗</span> BUILT TO MOVE YOU FORWARD</div><span className="coordinate">BB—01 / COLLECTIVE INTELLIGENCE</span></div>
...
<div className="stack-strip"><span>YOUR TOOLS.<br/><strong>OUR PLAYGROUND.</strong></span><div>HubSpot<span className="tool-symbol">✣</span></div><div className="ghl">HighLevel ↗</div><div className="shopify">▱ shopify</div><div className="webflow">≋ Webflow</div><div className="openai">◉ OpenAI</div><div>n8n <span className="tool-symbol">⌘</span></div></div>
<ReclaimSection/><section className="services section" id="services"><div className="section-top reveal"><span className="eyebrow">01 / THE CAPABILITIES</span><span className="muted">A few brilliant people. A whole lot of possibility.</span></div><div className="section-heading reveal"><h2>Less on your plate.<br/><span className="muted">More in your corner.</span></h2><a className="text-link" href="/services">Explore all services <Arrow/></a></div><div className="service-grid">{services.map((s,i)=><article className={'service-card card-'+i+' reveal'} key={s.category}><div className="card-top"><span className="eyebrow">0{i+1} / {s.category}</span><span className="service-symbol">{s.symbol}</span></div><div className={'service-art art-'+i} aria-hidden="true">{i===0?<><div className="code-window"><div><i/><i/><i/></div><pre><em>const</em> nextBigThing = {'{'}<br/>  vision: <b>"yours"</b>,<br/>  team: <b>"brainboys"</b>,<br/>  possibilities: <b>Infinity</b><br/>{'}'}</pre><span className="code-label">● READY TO SHIP</span></div><span className="floating-code">{'</>'}</span></>:i===1?<><div className="node n1">↗</div><div className="node n2">✧</div><div className="automation-core"><img src="/brainboys-mark.png" alt=""/></div><div className="node n3">✓</div><div className="node n4">⌘</div><div className="connection c1"/><div className="connection c2"/></>:<><div className="growth-orb"/><div className="growth-bar b1"/><div className="growth-bar b2"/><div className="growth-bar b3"/><div className="growth-arrow">↗</div><span className="growth-caption">MAKE YOURSELF<br/>IMPOSSIBLE TO IGNORE.</span></>}</div><h3>{s.title}</h3><p>{s.text}</p><div className="tags">{s.tags.map(t=><span key={t}>{t}</span>)}</div><a className="card-link" href={i===1?"/automation":"/services"}>Meet your {['builders','automators','creatives'][i]} <Arrow/></a></article>)}</div></section>
...
</> : path==='/automation'?<AutomationPage motion={motion}/>:page||<NotFound/>}</main><footer className="footer"><div className="footer-top"><span>BRILLIANT MINDS. ONE TEAM.</span><div><a href="/services">Services ↗</a><a href="/hire-a-team">Hire a team ↗</a><a href={contact}>Contact ↗</a></div><a href={isHome?"#home":"#main-content"}>BACK TO TOP ↑</a></div><div className="footer-word">brainboys<span>®</span></div><div className="footer-bottom"><span>© {new Date().getFullYear()} Brainboys AI</span><span>HUMAN INGENUITY. AMPLIFIED.</span><button aria-pressed={motion} onClick={()=>setMotion(!motion)}>Motion {motion?'on':'off'} <i className={motion?'on':''}/></button></div></footer>
</div>
}
export default App
